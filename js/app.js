...
// ---------- 미니 모드 ----------
// 안드로이드 Chrome 은 Document Picture-in-Picture API 를 지원하지 않아 별도 창을
// 띄우는 방식은 못 쓴다. 대신 삼성 "팝업 보기"(최근 앱 길게 누르기) 같은 OS 자체의
// 떠있는 작은 창 기능과 같이 쓰라고 만든 기능 — 영상 영역을 거의 안 보이게 줄이고
// (완전히 없애면 재생이 멈출 수 있어 몇 px 는 남겨둔다) 목록 버튼줄을 접어서,
// 좁고 낮은 창에서도 가사가 최대한 많이 보이게 한다.
function toggleCompact() {
  const on = appEl.classList.toggle("compact-mode");
  $("#compact-toggle").classList.toggle("on", on);
  $("#compact-toggle").textContent = on ? "원래대로" : "미니 모드";
